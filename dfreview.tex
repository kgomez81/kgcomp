...
\end{equation}
In steady state, the mean fitness increases by $s$ in the average establishment time $\langle \tau \rangle$, is also the average rate of increase of the population's mean fitness is the expected establishment time $\tau_q$.  Once $n_q(t)$ reaches size $1/qs$ and establishes, its growth becomes more and more deterministic due to selection.  For very large $t$, $n_q(t)$ is nearly deterministic and can be written as $n_q(t)= (1/qs)e^{qs(t-\tau)}$, where $tau$ is a time dependent stochastic variable.   
